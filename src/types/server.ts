 ...
 * developer-facing presentation entity.
 */
export interface DashboardServer {
  /** Unique composite snapshot identifier (e.g. "win-18240-3000") */
  id: string;
  ...
  pid: number;
  /** Executable binary name (e.g. "node.exe", "python.exe") */
  processName: string;
  ...
  /** Complete reconstructed process ancestry tree */
...
export type ServerSortField = 'port' | 'pid' | 'name' | 'runtime';

export type ServerSortDirection = 'asc' | 'desc';

export interface ServerFilterOptions {
  environment: string; // 'all' | 'windows' | 'wsl'
