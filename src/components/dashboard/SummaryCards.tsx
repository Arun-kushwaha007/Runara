...
export const SummaryCards: React.FC<SummaryCardsProps> = ({
  ...
  return (
    <section className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
      {/* 1. Running Servers */}
      <div className="bg-app-surface border border-app-border rounded-xl p-4 flex flex-col justify-between shadow-xs">
        <div className="flex items-center justify-between text-app-muted-fg">
          <span className="text-xs font-semibold uppercase tracking-wider text-app-muted-fg">
            Running Servers
          </span>
        ...
        <div className="mt-1.5 text-[11px] text-app-muted-fg">
      ...
      <div className="bg-app-surface border border-app-border rounded-xl p-4 flex flex-col justify-between shadow-xs">
        ...
        <div className="mt-2 text-2xl font-bold text-app-fg font-mono">
      ...
      <div className="bg-app-surface border border-app-border rounded-xl p-4 flex flex-col justify-between shadow-xs">
        <div className="flex items-center justify-between text-app-muted-fg">
          ...
          {runningDistros.length > 0 ? (
            <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 bg-emerald-500/15 border border-emerald-500/30 px-1.5 py-0.5 rounded">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
              <span>{runningDistros.length} Running</span>
            </span>
          ) : (
            <span className="text-[10px] font-semibold text-app-muted-fg bg-app-muted px-1.5 py-0.5 rounded border border-app-border-subtle">
        ...
        <div className="mt-1.5 text-[11px] text-app-muted-fg truncate" title={
